...
File: reset.py
Description: 
    - raw_data 파일의 config.json과 raw_filter.txt를 최초 생성하는 코드입니다.
    - config.json과 raw_filter.txt가 생성되지 않은 최초에만 실행해야 합니다. (생성 이후는 Update.py Airflow 로 관리)
    - reset.py 실행 이후, update.py를 한번 더 실행할 필요는 없습니다.
"""
...
def generate_new_config_dict():
    ...
    print("generate_new_config_dict: Title 5/7")
    ...
    print("generate_new_config_dict: Summary 6/7")
    new_page_id_to_summary_dict = get_new_page_id_to_summary_dict(new_config_dict)
    new_config_dict["page_id_to_summary_dict"] = new_page_id_to_summary_dict
    print("generate_new_config_dict: json Update 7/7")
    with open("raw_data/config.json", "w", encoding="utf-8") as file:
        json.dump(new_config_dict, file, indent=4, ensure_ascii=False)
    return new_config_dict


def generate_raw_filter(new_config_dict):
    print("generate_raw_filter: data_load 0/2")
    BASE_STR = """
    ...
    print("generate_raw_filter: rearrange 1/2")
    for main_page_id in page_hierarchy_dict:
        ...
        result += data_str
    print("generate_raw_filter: raw_filter.txt Update 2/2")
...
###########################################################################################
def run():
    new_config_dict = generate_new_config_dict()
    new_raw_filter = generate_raw_filter(new_config_dict)
    return
